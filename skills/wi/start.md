...
- **WI ID 자동 부여**: 일반적으로 `001`부터 순차 증가, zero-padded (예: 001, 002, ..., 099, 100)
- 자릿수: WI 총 개수에 따라 자동 결정 (1000개 이하 → 3자리, 1000개+ → 4자리)
- **영숫자 ID 사용자 지정 가능** (rules/wi-global.md 패턴 `[0-9A-Za-z]+(-[0-9]+)?`): 메이저 리팩토링 그룹 분류 시 `A2a`, `C3code`, `E1` 같은 영문 시작 ID, 후속 fix는 `001-1` / `A2a-1` 서브넘버링
